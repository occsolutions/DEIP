...
export default {
  methods: {
    $generateBurnoutIndexOrg () {
      const maxRowsPerPage = 8
      let overalCounter = 0
      const pages = []
      let index = 0
      const indexName = 'burnoutOrganizational'
      for (const segKey of Object.keys(this.segmentedAnswers)) {
        if (this.segmentedAnswers[segKey].count) {
          if (index === 0) {
            pages.push([
              // Page Title
              pdfUtils.generateHeaderTitle(this.$t('Views.Evaluations.report.toc.burnout_index_organizational'), overalCounter === 0, 600),
              // Headers Table
              {
